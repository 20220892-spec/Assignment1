...
stop("method must be forward/backward/central")
}
...
main="Analytical vs Numerical Derivative", ylab="f'(x)")
lines(xvals, num_val, col="red", lty=2)
legend("topright", legend=c("Analytical","Numerical"), col=c("blue","red"), lty=c(1,2))
...
ggplot(errs_long, aes(x=n, y=Error, color=Method)) +
geom_line(size=1.1) +
scale_y_log10() +
...
x <- backward(L, z)
x
solve(A,b)  # Compare with R built-in
...
p <- ggplot() +
geom_point(aes(x=model$X, y=model$y), color="gray60", alpha=0.5) +
